...
const signUp = async (req, res, next) => {
  ...
  try {
    ...
    if (!firstName || !lastName || !username || !password || !role || (role !== "trader" && role !== "user")) {
    ...
    if (user) {
      return next(
        ApiError.badRequest("This username is taken, please choose another one")
      );
    }
    ...
    if(role === "user"){
      ...
      const trader = await Trader.create({ UserId: newUser.id, storeName: storeName }, { transaction: t });
...
//     await newUser.setRole(roleObj);
//     const trader = await Trader.create({ UserId: newUser.id, storeName: storeName });
