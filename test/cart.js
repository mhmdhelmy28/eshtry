...
describe("Cart API", () => {
  ...
  before(async () => {
    ...
    describe("DELETE /cart", () => {
      ...
      before(async () => {
        ...
        cartId = cart.id;
        await CartItem.create({ CartId: cartId, UserId: user.id,ProductId: product1.id, quantity: 2 });
        await CartItem.create({ CartId: cartId, UserId: user.id,ProductId: product2.id, quantity: 1 });
      });
  ...
  describe("POST /cart/order", () => {
    ...
    before(async () => {
      const cart = await Cart.findOne({where: {UserId: user.id}});
      cartId = cart.id;
       item1 = await CartItem.create({ CartId: cartId, UserId: user.id,ProductId: product1.id, quantity: 2, price: product1.price * 2 });
       item2 = await CartItem.create({ CartId: cartId, UserId: user.id,ProductId: product2.id, quantity: 1, price: product2.price });
